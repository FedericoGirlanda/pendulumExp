...
class RoAController(AbstractController):
    ...
    def get_control_output(self, meas_pos, meas_vel,
        ...
        if self.active_controller == "lqr":
            ...
            cond1 = (np.round(meas_pos) == np.round(self.x_i[0])) and (np.round(meas_vel,1) == np.round(self.x_i[1],1)) 
            if cond1:
                self.active_controller = "wait"
                self.change_time = meas_time
                print("waiting...")
        elif self.active_controller == "wait":
            des_pos, des_vel, u = self.lqr.get_control_output(meas_pos, meas_vel, meas_tau, meas_time)
            ...
            if self.wait_time < 0:
                ...
                print("tvlqr action...")
        elif self.active_controller == "manualInit":
            ...
            if cond1:
                ...
                self.tvlqrTime = meas_time
                print("tvlqr action...")
        elif self.active_controller == "tvlqr":
